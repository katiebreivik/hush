...
\textbf{Figure \ref{fig:met_dep_porb}, shows the companion frequency f$_{\rm{logP}}$ for solar-type stars as a function of the logarithm of orbital period, mirroring the inner binaries curve of Figure 2 in \citet{Moe2021} and scaling to an overall multiplicity frequency of 0.67 (equivalent to an integrated binary fraction of 50\%) at solar metallicity. To obtain the normalization of the companion frequency for close binaries ($\log_{10}(P_{\rm{orb}}/\rm{day}<10^4$)),} we fit the results presented in \citet{Moe2019} using linear regression to obtain a piecewise relation between the metallicity, [Fe/H] and \text{close} binary fraction $f_{\rm{b}}$ as
...
\begin{figure*}
    ...
    \caption{The DWD formation efficiency vs metallicity of DWD populations simulated with \cosmic. Each panel shows the formation efficiency for a given DWD type \snotes{and variation}. The solid lines indicate the formation efficiency for model FZ which incorporates a metallicity-dependent binary fraction. The dashed lines indicate the formation efficiency for model F50, which assumes a constant binary fraction of $50\%$. The DWD formation efficiency drops by a factor of $4-5$ for model FZ and a factor of \snotes{\sout{2-5} 1-5} for model F50. See Section~\ref{sec:formeff} for a careful description of the trends for each DWD type.}
    \label{fig:form_eff}
    \script{form_eff.py}
\end{figure*}
...
\begin{figure*}
    ...
    \caption{Average interaction separation, $\overline{\mathrm{a}}_{\rm{CE}}$ of progenitors of close DWDs across metallicity for each DWD type \snotes{and variation} from model FZ. Solid lines show the average separation at the first RLO for binaries in each metallicity bin. The shaded regions show the $1\sigma$ spread around the mean within each metallicity bin. The average interaction separation increases with metallicity for every DWD type\snotes{, except for $\alpha5$ which remains approximately constant for CO + He and ONe + X}. The positive trend in the average interaction separation is a direct consequence of larger envelope masses of higher-metallicity donors which are less evolved than their lower-metallicity counterparts. The sharp drop of the CO + CO average at the highest-metallicity bin \snotes{of the fiducial simulation} is due to binning effects of the figure: it only contains 115 systems, of which the majority stem from a single low-separation system which was sampled 94 times times during the simulation.} 
    \label{fig:CEsep}
    \script{CEsep.py}
\end{figure*}
...
For CO + He DWD progenitor systems with intermediate orbital periods ($1.5\leq \log_{10}(P_{\rm{orb}}/\rm{day})< 2.5$) the mechanisms which impact formation efficiency are complex. The dominant way to % R: lose
...
\snotes{For binary systems leading to He + He DWDs, there are many more mergers for $\alpha25$ than for the fiducial \kb{model}. The lower CE efficiency \kb{\sout{entails} leads to DWD progenitor} mergers from \kb{failed CE ejection \sout{the binary failing to eject the CE}}, or \kb{the CE} shrinking the orbit such that it then merges on the second CE phase. Because\kb{\sout{, as discussed above,}} there are more \kb{\sout{very short-period} ZAMS binaries with orbital periods shorter than $10^4$ days} in F50, as the \kb{\sout{ZAMS binary}} population evolves there are more likely to be binary interactions in the population for F50 than FZ. This leads to more mergers in $\alpha25$ occurring for F50 before formation of the DWD and thus a lower formation efficiency overall for F50 in contrast to FZ. \kb{\sout{as we see} This is illustrated} in the $\alpha25$ He + He plot of Figure \ref{fig:form_eff}. Similar to the fiducial \kb{model}, a binary that becomes a He + He DWD at lower metallicities would merge before becoming one at higher metallicities because of the longer evolution timescales. These later interactions lead to closer post-CE separations and thus more mergers. The dip at [Fe/H]=$\sim -1.5\%$ stems from mergers before the DWD forms, \kb{again} similar to \kb{sout{for our}the } fiducial model. The peak that occurs in $\alpha25$ at $\sim 10\%$ Z$_\odot$ arises from binaries evolving faster \kb{\sout{(i.e.} and thus}} interacting earlier as metallicity increases. \kb{This means \sout{meaning}} that equivalent binaries that would have merged or formed a CO + He DWD instead at lower metallicities now survive \kb{\sout{into} to produce} a He + He \kb{DWD}. At higher metallicities, there are a higher proportion of wide\kb{\sout{-separation systems} ZAMS binaries}, which evolve off the Main Sequence too slowly to form a He + He DWD, causing the efficiency drop.

\snotes{For $\alpha5$, there are significantly fewer \kb{DWD progenitor} mergers, \kb{\sout{as expected}} since binaries are more likely to survive a CE phase \kb{\sout{from} due to} the higher CE \kb{ejection} efficiency. Systems that would normally merge for $\alpha25$ \kb{\sout{now} instead} survive, and are shrunk to short orbits through CE interactions to \kb{\sout{evolve} reside} in the LISA band \kb{at present}. The number of DWDs thus makes up a higher fraction of the total simulated binary population \kb{\sout{overall}}, thus we see \kb{\sout{in general}} higher formation efficiencies than for \kb{\sout{our} the} fiducial \kb{\sout{simulations} model} (\kb{\sout{which has }}$\alpha=1$). \kb{\sout{, and even higher than for our} This effect is even more pronouced in comparison with the $\alpha25$} variation \kb{\sout{with} which sets} $\alpha=0.25$. }

\snotes{Beyond \kb{\sout{this} the effects described above}, the efficiency drop across metallicity is similar to the fiducial \kb{model} in that it depends on the metallicity-dependent ZAMS orbital period distribution emphasizing short-period systems at low metallicities, and in the change of formation channel for different period ranges across metallicity. We \kb{\sout{see} find} the highest formation efficiency for variation $q3$. Only the shortest-period binaries do not form a DWD through mergers at higher metallicities. At low metallicities, \kb{\sout{we see that}} even binaries with \kb{\sout{very close}} ZAMS separations \kb{less than XX} form He + He DWDs. \kb{\sout{We}} Thus \kb{\sout{see} this leads to} a higher \kb{formation} efficiency from F50 over FZ for parameter variation $q3$ because of the higher number of shortest-period ZAMS binaries in the log-uniform distribution of F50.}

\snotes{For CO + He binaries, there are again many more mergers for \kb{parameter variation} $\alpha25$ relative to the fiducial case. This is the \kb{\sout{dominating} dominant} way that \kb{progenitors do not evolve to produce DWDs \sout{binaries are lost}} at higher metallicities. At low metallicities \kb{\sout{we also see}} a number of \kb{\sout{systems} progenitor binaries} form CO + CO \kb{\sout{binaries} DWDs instead of He + CO DWDs}. \kb{\sout{-- t}T}his is especially the case in \kb{\sout{a ``Goldilocks zone" around} near} log$_{10}$(Z/Z$_\odot)\sim -2$, where RLO occurs late enough in \kb{a binary's} evolution that mass transfer remains stable and CO + CO's are formed rather than CO + He, hence \kb{\sout{why we see} producing} a dip in the formation efficiency. Because \kb{parameter variation} $\alpha5$ \kb{\sout{entails leads to}} less mergers due to the high CE efficiency, the formation efficiency is mostly governed by the metallicity-dependent ZAMS orbital period distribution. At higher metallicities the majority of \kb{\sout{lost systems} cases in model FZ where progenitors don't produce a CO + He DWD} are due to \kb{progenitor} mergers or to He + He \kb{\sout{binaries} DWDs} forming instead \kb{CO + He DWDs.}} %due to the faster evolution at high metallicities and increasing radii inducing earlier binary interactions and hence stripping of both star's envelopes [$\leftarrow$ is this correct logic?]. I think this is already discussed above for fiducial so we don't need to worry about it.
\snotes{At higher metallicities, the ZAMS orbital period distribution of FZ also drops below the \kb{orbital period} distribution \kb{\sout{for} of} F50 and extends out to orbital periods beyond what would form a CO + He. \kb{This causes \sout{ which is why}} the formation efficiencies for FZ and F50 \kb{to} cross at log$_{10}$(Z/Z$_\odot$)$\sim-0.5$. \kb{This is also apparent \sout{which we also see}} for the $q3$ variation, and to a lesser extent in the fiducial \kb{\sout{variation} case}.} %Any lost systems in $q3$ also is contributed to by: (i) some binaries at high metallicities not forming a CO + He DWD by present day due to envelope-stripping timescales being delayed by phases of stable mass transfer, (ii) mergers, and (ii) ONe + X DWDs forming instead.
\snotes{In $q3$, for any given orbital period we find that lower mass \kb{\sout{systems} DWDs originate \sout{come}} from lower metallicities,\kb{\sout{which leads} thus leading} to an approximately constant formation efficiency.}

\snotes{For CO + CO, across all variations, very close \kb{ZAMS} binaries (P$_{\rm{orb}} \mathcal{O}(100$ days)) at high metallicities} \kb{form a CO + He or He + He DWD instead of a CO + CO because the binary interacts before core Helium burning of one or both components. } 
%lost at high metallicities when a binary that would form a CO + CO at low metallicities forms a CO + He or He + He DWD instead 
There are again many more mergers in $\alpha25$, and \kb{\sout{less} fewer} mergers for $\alpha5$. Due to the high CE efficiency of $\alpha5$, systems with shorter ZAMS orbital periods survive to become CO + CO DWDs rather than merging or becoming CO + He or He + He DWDs. There are \kb{\sout{less} fewer} mergers overall for $q3$ than all the $\alpha$ \kb{\sout{values} variations} because \kb{\sout{of the phase of stable mass transfer occurring before the CE phase} the first RLO occurrence leads to stable mass transfer from the primary to the secondary} rather than multiple CE phases. \kb{For both the $\alpha5$ and $q3$ variations, the majority of CO + CO DWDs originate in wide enough orbits that they don't interact before DWD formation. This leads to an approximately constant formation efficiency across metllicity in both cases.}

%\kb{This also leads to \kb\souce{hence why} \kb{an approximately constant formation efficiency} $q3$ \sout{efficiency}\kb{parameter variation} is approximately constant across metallicity.}
%\kb{**I think maybe another thing to point out is that the majority of CO + CO don't interact }

\kb{For model FZ, the ONe + X formation efficiencies follow similar trends aross each parameter variation. Their formation scenarios are similar to those described above for the fiducial binary evolution parameter set. The main difference between each parameter variation is only in the number of sources produced since the number of stellar mergers that occur before DWD formation increases with decreasing common envelope efficiency. In model F50, the formation efficiency is decreased significantly for parameter variation $\alpha25$ because the log-uniform orbital period distribution produces many ZAMS binaries with very short orbital periods which lead to stellar mergers during the CE phases.}

% Reworked this into the paragraph above.
%\snotes{[** Might need to fact check this paragraph **] The FZ curves for ONe + X are relatively similar amongst variations beyond height, and the loss of systems is the same as described for the fiducial simulation. We have more mergers once again for $\alpha25$, which lowers the F50 curve below FZ because F50 has the shortest-ZAMS-period end that FZ does not have. $\alpha5$ and $q3$ show F50 efficiencies with a very shallow slope due to the flat orbital period distribution, as well as due to less mergers overall.}
...
\begin{figure*}
    ...
    \caption{The number of LISA-band systems formed for each DWD type as a function of the base-10 logarithm of metallicity, normalized to solar value. The solid line shows the FZ population with a metallicity-dependent \snotes{close} binary fraction incorporated, and the dashed line shows the F50 population for a standard binary fraction of 0.5. The LISA population of DWDs is dominated by stars with super-solar metallicities. This is true even for model FZ, which drops off significantly for higher metallicities, because of the large number of stars formed in \textbf{m12i} beyond Z$\simeq$Z$_\odot$. There is a double peak in the \snotes{fiducial and $\alpha25$ }He + He population\snotes{s}; the first peak is caused by the sharp drop in formation efficiency past Z$\simeq$0.1$Z_\odot$ which is then greatly overcompensated for by the amount of star formation at higher metallicities which forms the second peak.}
    \label{fig:lisa_nums}
    \script{lisa_nums.py}
\end{figure*}
...
We note that the height of the confusion foreground and the number of DWDs radiating GWs with $f_{\rm{GW}} > 10^{-4}\,\rm{Hz}$ is reduced by a factor of 2 for model FZ relative to model F50\snotes{ for variations $\alpha5$ and $q3$}, the number of resolved sources is not reduced to an equal degree. \snotes{This is because in the absence of less competing GW signals from DWDs in the LISA frequency band, more DWDs can be individually resolved. For the fiducial and $\alpha25$ variations, the number of resolvable systems with SNR $>7$ \kb{\sout{do not change much}} are reduced by 4.5\% and increased by 3\% respectively. However, for $\alpha5$ the number of resolvable DWDs is reduced by only 26\%, and the number of resolvable DWDs $q3$ remains almost \emph{unchanged}, with a 1\% reduction even though the overall LISA population of both variations is halved. A large portion of the maintenance of the number of DWDs in parameter variation $q3$ is contributed to by He + He binaries with higher GW frequencies and lower ASDs which can now survive CE interactions due to an initial phase of mass transfer (see discussion of this formation channel in Section \ref{sec:CEsep}).} 

%\snotes{[\emph{This is because in the absence of less competing GW signals from DWDs in the LISA frequency band, more DWDs can be individually resolved.}][$\leftarrow$ moved to beginning of paragraph] Strikeout: [\emph{Between models FZ and F50, the number of resolvable systems with SNR $>7$ over all DWD types decreases by only $\sim14\%$.}]}


The distance and chirp mass of DWDs which exhibit observable orbital evolution due to the emission of GWs during the LISA mission can be measured. This is because the chirp mass -- distance degeneracy in the observed strain can be broken with the observed GW frequency evolution, or chirp. Assuming a chirp resolution of $1/T_{\rm{obs}}^2 \sim 8\times10^{-9}\,\rm{Hz}^2$, we select the DWDs whose chirp masses and distances can be measured. Figure \ref{fig:Mc_vs_dist} shows the chirp mass vs the luminosity distance for each DWD type in this selected population \snotes{for the fiducial simulation. The same figure for our three variations is shown in Appendix \ref{appendix:Mc_dist_vars}}. The contours show the $5^{\rm{th}}$, $25^{\rm{th}}$, $50^{\rm{th}}$, $75^{\rm{th}}$, and $95^{\rm{th}}$ percentiles for models FZ (light blue) and F50 (dark blue, dashed). Despite the reduction in the height of the confusion foreground when considering model FZ relative to F50, we find that LISA is unable to differentiate between the chirp mass -- distance distributions of the two models. \snotes{For $\alpha25$ we plot these as scatter plots for He + He and ONe + X due to low statistics since there are so few binaries in this selected population. The only population with a significant change between models is for ONe + X's $\alpha5$ distributions, where we see a skew to lower masses. This is because a higher fraction of the binaries are ONe + He (in which Helium WDs tend to have smaller masses) for FZ than F50 -- a Helium companion makes up 10\% of this selected population for F50 compared with 44\% for FZ. Furthermore, $\sim8$\% of the F50 measurable systems are ONe + ONe binaries, versus 0\% for FZ. Because ONe WDs have larger masses, this also creates the high-mass concentration we see in the F50 contours.}

\section{\snotes{\sout{Binary evolution \textbf{parameter variations}} Galactic DWD Confusion Foreground Discussion}}
\label{sec:model_compare}


\begin{figure}
	\includegraphics[width=0.45\textwidth]{figures/model_comp.pdf}
    \caption{The top panel shows the Galactic DWD confusion fit vs GW frequency for different binary evolution \textbf{parameter variations} (colors) with model F50 shown in dashed lines and model FZ shown in solid lines. The bottom panel shows the ratio of the number of DWDs orbiting in the LISA frequency band for model FZ to model F50 vs to number of DWDs orbiting in the LISA band for model F50 only. While both the height of the confusion foreground and number of LISA DWDs changes for each variation, the FZ models within each variation exhibit \snotes{either no change or }a constant reduction by a factor of two compared with the F50 models.}
