...
\snotes{For CO + He binaries, there are again many more mergers for \kb{parameter variation} $\alpha25$ relative to the fiducial case. This is the \kb{\sout{dominating} dominant} way that \kb{progenitors do not evolve to produce DWDs \sout{binaries are lost}} at higher metallicities. At low metallicities \kb{\sout{we also see}} a number of \kb{\sout{systems} progenitor binaries} form CO + CO \kb{\sout{binaries} DWDs instead of He + CO DWDs}. \kb{\sout{-- t}T}his is especially the case in a ``Goldilocks zone" around log$_{10}$(Z/Z$_\odot$)sim -2, where RLO occurs late enough in \kb{a binary's} evolution that mass transfer remains stable and CO + CO's are formed rather than CO + He, hence \kb{\sout{why we see} producing} a dip in the formation efficiency. Because \kb{parameter variation} $\alpha5$ \kb{\sout{entails leads to}} less mergers due to the high CE efficiency, the formation efficiency is mostly governed by the metallicity-dependent ZAMS orbital period distribution. At higher metallicities the majority of \kb{\sout{lost systems} cases in model FZ where progenitors don't produce a CO + He DWD} are due to \kb{progenitor} mergers or to He + He \kb{\sout{binaries} DWDs} forming instead \kb{CO + He DWDs.} %due to the faster evolution at high metallicities and increasing radii inducing earlier binary interactions and hence stripping of both star's envelopes [$\leftarrow$ is this correct logic?]. I think this is already discussed above for fiducial so we don't need to worry about it.
At higher metallicities, the ZAMS orbital period distribution of FZ also drops below the \kb{orbital period} distribution \kb{\sout{for} of} F50 and extends out to orbital periods beyond what would form a CO + He. \kb{This causes \sout{ which is why}} the formation efficiencies for FZ and F50 \kb{to} cross at log$_{10}$(Z/Z$_\odot$)$\sim-0.5$. \kb{This is also apparent \sout{which we also see}} for the $q3$ variation, and to a lesser extent in the fiducial \kb{\sout{variation} case}.} %Any lost systems in $q3$ also is contributed to by: (i) some binaries at high metallicities not forming a CO + He DWD by present day due to envelope-stripping timescales being delayed by phases of stable mass transfer, (ii) mergers, and (ii) ONe + X DWDs forming instead.
\snotes{In $q3$, for any given orbital period we find that lower mass systems come from lower metallicities, which leads to an approximately constant formation efficiency.}

\snotes{For CO + CO, across all variations, very close \kb{ZAMS} binaries (P$_{\rm{orb}} \mathcal{O}(100$ days)) are lost at high metallicities when a binary that would form a CO + CO at low metallicities forms a CO + He or He + He DWD instead because the binary interacts before core Helium burning of one or both components. There are again many more mergers in $\alpha25$, and less mergers for $\alpha5$. Due to the high CE efficiency of $\alpha5$, systems with shorter ZAMS orbital periods survive to become CO + CO DWDs rather than merging or becoming CO/He + He DWDs. There are many less mergers overall for $q3$ than all the $\alpha$ values because of the phase of stable mass transfer occurring before the CE phase rather than multiple CE phases, hence why the $q3$ efficiency is approximately constant across metallicity.}
...
We note that the height of the confusion foreground and the number of DWDs radiating GWs with $f_{\rm{GW}} > 10^{-4}\,\rm{Hz}$ is reduced by a factor of 2 for model FZ relative to model F50\snotes{ for variations $\alpha5$ and $q3$}, the number of resolved sources is not reduced to an equal degree. \snotes{This is because in the absence of less competing GW signals from DWDs in the LISA frequency band, more DWDs can be individually resolved. For the fiducial model and $\alpha25$ variation, the number of resolvable systems with SNR $>7$ do not change much (are reduced by 4.5\% and increased by 3\% respectively). However, $\alpha5$ sees a reduction in the number of resolvable systems by only 26\%, and $q3$ remains almost \emph{unchanged}, seeing a 1\% reduction, even though the overall LISA population of both variations is halved. A large portion of the maintenance of $q3$ is contributed to by He + He binaries with higher GW frequencies and lower ASDs which can now survive CE interactions due to an initial phase of mass transfer (see discussion of this formation channel in Section \ref{sec:CEsep}).} \snotes{[\emph{This is because in the absence of less competing GW signals from DWDs in the LISA frequency band, more DWDs can be individually resolved.}][$\leftarrow$ moved to beginning of paragraph] Strikeout: [\emph{Between models FZ and F50, the number of resolvable systems with SNR $>7$ over all DWD types decreases by only $\sim14\%$.}]}
