...
def save_full_galaxy(
    DWD_list, pathtodat, fire_path, pathtosave, interfile, model, nproc
):
    """
    ...
    if len(dwd_eff_dat[0][0]) > 1:
        dwd_eff_dat = pd.DataFrame(
            dwd_eff_dat, columns=["form_eff", "label", "met", "binfrac"]
        )
        ...
        for var in [True, False]:
            ...
            for DWD in DWD_list:
                ...
                if var:
                    dwd_eff_select = dwd_eff_dat.loc[
                        (dwd_eff_dat.binfrac < 0.5) & (dwd_eff_dat.label == label)
                    ]
                    bin_var = "FZ"
                else:
                    dwd_eff_select = dwd_eff_dat.loc[
            ...
            DWDeff.to_hdf(
                pathtosave + "results.hdf", key="DWDeff_{}_{}".format(bin_var, model)
            )

    return


def get_interactionsep_and_numLISA(
    pathtodat,
    pathtosave,
    model,
    var,
    ...
    numLISA_30bins = pd.DataFrame(
        np.array(num_LISA).T, columns=["He", "COHe", "CO", "ONe"]
    )
    if var:
...
def get_resolvedDWDs(pathtodat, pathtosave, var, model, window=1000):
    ...
    def cosmic_confusion_var(
        f, L, t_obs=4 * u.yr, approximate_R=True, confusion_noise=None
    ):
        power_dat = pd.read_hdf(
            paths.data / "results.hdf",
            key="total_power_DWDs_{}_{}".format("FZ", "fiducial"),
        )

        power_dat_median = power_dat.rolling(window).median()
        power_dat_median = power_dat_median[window:]

        power_dat_median_fit = power_dat_median.loc[
            (power_dat_median.strain_2 > 0) & (power_dat_median.f_gw <= 1.2e-3)
        ]
    ...
    def cosmic_confusion_50(
        f, L, t_obs=4 * u.yr, approximate_R=True, confusion_noise=None
    ):
        power_dat = pd.read_hdf(
            paths.data / "results.hdf",
            key="total_power_DWDs_{}_{}".format("F50", "fiducial"),
        )
    ...
    for kstar1, kstar2 in zip(kstar1_list, kstar2_list):
        for i in range(len(met_arr) - 1):
            ...
            if len(dat) == 0:
                ...
                dat = Ldat[
                    ["mass_1", "mass_2", "dist_sun", "f_gw", "kstar_1", "kstar_2"]
                ]
            else:
                Ldat = pd.read_hdf(pathtodat + Lsavefile, key=Lkey)
                dat = dat.append(
                    Ldat[["mass_1", "mass_2", "dist_sun", "f_gw", "kstar_1", "kstar_2"]]
                )

    sources = source.Source(
        m_1=dat.mass_1.values * u.Msun,
    ...
    power_dat = pd.DataFrame(
        np.vstack([lisa_bins, power_foreground]).T, columns=["f_gw", "strain_2"]
    )
    ...
    if model == "fiducial":
        ...
        dat["snr"] = sources_conf.get_snr(
            t_obs=Tobs, verbose=False, custom_psd=cosmic_confusion
        )
        dat["chirp"] = (
            utils.fn_dot(sources_conf.m_c, sources_conf.f_orb, sources_conf.ecc, n=2)
            .to(u.s ** (-2))
            .value
        )
...
def get_formeff(pathtodat, pathtosave):
    ...
    def formeff(datfiles, pathtodat, model):
        ...
        for i in range(15):
            if model == "F50":
                ...
                ratio = ratios[i]
            mass_binaries = pd.read_hdf(pathtodat + datfiles[i], key="mass_stars").iloc[
                -1
            ]
    ...
    DWDeff.to_hdf(paths.data / "results.hdf", key="DWDeff_{}_{}".format("FZ", "fiducial"))
    DWDeff05.to_hdf(paths.data / "results.hdf", key="DWDeff_{}_{}".format("F50", "fiducial"))
